...
- Nếu có `roomCode`, API sẽ trả về danh sách nhân vật chuẩn cùng `takenCharacters` đang được dùng trong phòng đó
- UI dùng `takenCharacters` để khóa nút, còn khi thoát phòng hoặc room bị đóng thì danh sách lại mở đầy đủ
...
- Khi gọi lại danh sách nhân vật theo phòng, nhân vật đó sẽ không còn nằm trong `takenCharacters`
...
- `GameManager.SelectedCharacter` chỉ còn là state cho single-player

#### Tác dụng
- Single player vẫn dùng `selectedCharacter` ở mức tài khoản như cũ
- Multiplayer room dùng lựa chọn riêng trong `RoomPlayer`
- Thoát phòng hoặc room bị đóng sẽ trả lại quyền chọn nhân vật
